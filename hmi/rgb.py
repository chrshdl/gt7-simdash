    ...
    def update(self, data):
        if data.flags.rev_limiter_alert_active:
            ...
            if data.engine_rpm < 100:
                self.clear_all_rgb()
            elif data.engine_rpm in range(
                int(0.75 * data.rpm_alert.min), int(0.83 * data.rpm_alert.min)
            ):
                self.show_rgb(2)
            elif data.engine_rpm in range(
                int(0.83 * data.rpm_alert.min), int(0.92 * data.rpm_alert.min)
            ):
                self.show_rgb(4)
            elif data.engine_rpm in range(
                int(0.92 * data.rpm_alert.min), int(data.rpm_alert.min)
            ):
                self.show_rgb(6)
            elif data.engine_rpm in range(
                int(data.rpm_alert.min), int(data.rpm_alert.max)
            ):
                self.show_rgb(8)
